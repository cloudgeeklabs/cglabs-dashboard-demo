...
var roleAssignmentObject = {
  role1: {
    name: guid('readerRole',subscription().id) // has to be globally unique - thus using resourceGroup().id as part of GUID
  ...
  role2: {
    name: guid('monitorReaderRole',subscription().id) // has to be globally unique - thus using resourceGroup().id as part of GUID
  ...
  role3: {
    name: guid('grafanaAdmin',subscription().id) // has to be globally unique - thus using resourceGroup().id as part of GUID
